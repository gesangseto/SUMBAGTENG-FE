...
const ListDepartment = () => {
  ...
  const handleClickCreate = (val) => {
    history.push(`/administrator/department/create`);
  };
  const handleClickRead = (item) => {
    history.push(`/administrator/department/read/${item.department_id}`);
  };
  const handleClickUpdate = (item) => {
    history.push(`/administrator/department/update/${item.department_id}`);
  };
    ...
    confirmAlert({
      ...
      buttons: [
        ...
        },
        {
          label: "Yes",
          onClick: () => {
            var body = { data: { department_id: item.department_id } };
            console.log(body);
            $axios.delete(`master/user_department`, body).then((res) => {
