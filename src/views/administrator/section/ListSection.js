...
const ListSection = () => {
  ...
  const handleClickCreate = (val) => {
    history.push(`/administrator/section/create`);
  };
  const handleClickRead = (item) => {
    history.push(`/administrator/section/read/${item.section_id}`);
  };
  const handleClickUpdate = (item) => {
    history.push(`/administrator/section/update/${item.section_id}`);
  };
